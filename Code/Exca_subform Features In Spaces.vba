...
Option Explicit
Private Sub CmdDelSpaceRelation_Click()
On Error GoTo Err_CmdDelSpaceRelation_Click
    Dim stDocName As String
    Dim stLinkCriteria As String
    Dim msg, retval, sql, permiss
    If Not IsNull(Me![txtIn_Space]) Or Me![txtIn_Space] <> "" Then
            permiss = GetGeneralPermissions
            If permiss = "ADMIN" Or permiss = "exsuper" Then
                msg = "Do you really want to delete the relation to this space?"
                retval = MsgBox(msg, vbInformation + vbYesNo, "Space Number does not exist")
                If retval = vbNo Then
                Else
                    sql = "DELETE FROM [Exca: Features in Spaces] WHERE [In_Space]=" & Me![txtIn_Space] & " AND [Feature]=" & Me![Feature]
                    DoCmd.RunSQL sql
                End If
            Else
                MsgBox "Sorry but you have not the permission to delete this record"
            End If
    Else
        MsgBox "This space number is actually not related to the feature."
    End If
    Me.Requery
Exit_CmdDelSpaceRelation_Click:
    Exit Sub
Err_CmdDelSpaceRelation_Click:
    Call General_Error_Trap
    Resume Exit_CmdDelSpaceRelation_Click
End Sub
...
On Error GoTo err_lost
    Forms![Exca: Feature Sheet]![Exca: subform  Features in Buildings].Form.Requery
Exit Sub
err_lost:
    Call General_Error_Trap
    Exit Sub
